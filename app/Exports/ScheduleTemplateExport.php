...
class ScheduleTemplateExport implements FromArray, WithHeadings, WithEvents
{
    ...
    public function __construct()
    {
        ...
        $this->rest_days = [
            ...
            ['val' => 'M', 'name' => 'Monday'],
            ['val' => 'T', 'name' => 'Tuesday'],
            ['val' => 'W', 'name' => 'Wednesday'],
            ['val' => 'TH', 'name' => 'Thursday'],
            ['val' => 'F', 'name' => 'Friday'],
            ['val' => 'SA', 'name' => 'Saturday'],
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getColumnDimension('K')->setWidth(15); // rest_day
                $sheet->getColumnDimension('L')->setWidth(15); // rest_day_id
                ...
                $restDaysSheet = $spreadsheet->createSheet();
                $restDaysSheet->setTitle('RestDays');
                foreach ($this->rest_days as $index => $day) {
                    $row = $index + 1;
                    $restDaysSheet->setCellValue("A{$row}", $day['name']);
                    $restDaysSheet->setCellValue("B{$row}", $day['val']);
                }


                $spreadsheet->getSheetByName('RestDays')->setSheetState(Worksheet::SHEETSTATE_HIDDEN);
                ...
                for ($i = 2; $i <= 100; $i++) {
                    ...
                    // Set dropdown for rest days
                    $restDayValidation = $sheet->getCell($restDayCell)->getDataValidation();
                    $restDayValidation->setType(DataValidation::TYPE_LIST);
                    $restDayValidation->setErrorStyle(DataValidation::STYLE_STOP);
                    $restDayValidation->setAllowBlank(true);
                    $restDayValidation->setShowInputMessage(true);
                    $restDayValidation->setShowErrorMessage(true);
                    $restDayValidation->setShowDropDown(true);
                    $restDayValidation->setFormula1("=RestDays!\$A\$1:\$A\$" . count($this->rest_days));
                    // Set VLOOKUP formula for rest day ID
                    $sheet->setCellValue($restDayIdCell, "=IFERROR(VLOOKUP(K{$i},RestDays!A:B,2,FALSE), \"\")");
                    // Protect rest_day_id column
                    $sheet->getStyle($restDayIdCell)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                }
                ...
                for ($i = 2; $i <= 100; $i++) {
                    ...
                    $sheet->getStyle("L{$i}")->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                }
                ...
                foreach (range('I', 'I') as $col) { // Up to column I now
                    for ($row = 2; $row <= 100; $row++) {
                        $sheet->getStyle("{$col}{$row}")->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                    }
                }

                foreach (range('K', 'K') as $col) { // Up to column I now
                    for ($row = 2; $row <= 100; $row++) {
                        $sheet->getStyle("{$col}{$row}")->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                    }
                }
